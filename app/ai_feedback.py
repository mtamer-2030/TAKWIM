...
def _generate(system_prompt: str, user_prompt: str, fmt: str | None = None,
              num_predict: int = 350, num_ctx: int | None = None,
    ...
    timeout: مهلة الطلب بالثواني؛ تُرفَع للاستخراج المطوّل على المعالج (بلا بطاقة رسوم).
    """
    cfg = settings.local_ai
    if not cfg.enabled:
    ...
    try:
        resp = httpx.post(f"{cfg.base_url}/api/generate", json=payload,
...
# سقفُ توليدِ الخطط: العربيّةُ تُنتج توكناتٍ كثيرةً لكلّ كلمة، فخطّةٌ من ٣ نقاطٍ مفصّلة
# قد تتجاوز ٩٠٠ توكناً فتُقتطع في منتصف الجملة («تقارير مبتورة»). نجعله سخيّاً (٢٠٤٨)
# فلا يُبتر أبداً — والنموذج يتوقّف طبيعيّاً (done_reason=stop) قبل بلوغه فلا يبطئ إلّا
# إن احتاج فعلاً. المهلة ٣٠٠ث كافية والتوليدُ مسائيٌّ دفعيّ. لا يمسّ التصحيح المُعان.
PLAN_NUM_PREDICT = 2048
PLAN_NUM_CTX = 4096
...
# ————————— الاستخراج الذكيّ: هيكلة فرضٍ من نصّه الخام (اقتراح يراجعه الأستاذ) —————————

SYSTEM_EXTRACT = (
    "أنت مساعد أستاذ فلسفة مغربيّ خبير. حوّل نصّ الفرض/التمرين إلى JSON مهيكل يمثّل "
    "عناصره بالترتيب. أعِد JSON فقط بلا أيّ شرح، بهذا المخطّط:\n"
    '{"title":"عنوان الفرض","questions":[{"type":"...","prompt":"...","max_score":3,'
    '"options":["..."],"correct":[0]}]}\n'
    "قواعد صارمة:\n"
    "- type من: heading (عنوان قسم يُعرَض) · passage (نصّ للقراءة يُعرَض) · "
    "mcq_single (اختيار واحد) · mcq_multi (اختيار متعدّد) · long_text (مقال/تحليل) · "
    "short_text (جواب قصير).\n"
    "- احذف أسطر الإجابة الفارغة (نقاط أو خطوط)، والترويسات الإداريّة، والتذييل (مثل «والله وليّ التوفيق»).\n"
